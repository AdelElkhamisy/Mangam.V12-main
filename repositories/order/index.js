...
module.exports = class {
  ...
  async getAll(id, { type }) {
    ...
    const [images, customerReviews, orderReviews, propsArr, propsValues] =
      await Promise.all([
        ...
        doQuery("SELECT * FROM product_props_value WHERE id IN (?)", [orderIds])
      ]);
  ...
  async changeStatus(data) {
    ...
    let offers = await doQuery("SELECT * FROM offers WHERE id = ?", [
      order[0]?.offer_id
    ]);

    try {
      if (offers[0]?.isTendered && data?.success && status == "pending") {
        await doQuery("UPDATE offers SET success = ?, notes = ? WHERE id = ?", [
          1,
          order[0]?.userName + " -- succeeded in purchasing this offer",
          offers[0]?.id
        ]);
      }
    } catch (e) {
    ...
    offers = await doQuery("SELECT * FROM offers WHERE id = ?", [
      order[0]?.offer_id
    ]);
  ...
  async report(data) {
    const insertOrder = await doQuery(
      "INSERT INTO reports SET user_id = ?, order_id = ?, text = ?, user_phone = ?, user_email=?, user_name=?",
      [
        data?.userId,
        data?.orderId,
        data?.text,
  ...
  async replyReport(data) {
    const insertOrder = await doQuery(
      "UPDATE reports SET reed = ? WHERE id = ?",
      [true, data?.report_id]
    );
  ...
  async getAllReports(req, res) {
    const reports = await doQuery("SELECT * FROM reports");
      ...
      await Promise.all(
        reports.map(async (item, index) => {
          let {
            ...
            props_values,
            shipping
          } = await this.getOrderDetails(item.user_id, item.order_id);
